...
def image_qa_prompt() -> str:
    return (
        "You are a ruthless text-proofreader for an AI-generated image. "
        "Diffuse models frequently garble text - assume text is WRONG until "
        "you have verified it letter by letter.\n\n"
        "STEP 1 - TRANSCRIBE: List every separate visible text fragment exactly "
        "as written, letter for letter. Include titles, labels, bullets, signs, "
        "and anything inside speech bubbles. Count them.\n"
        "STEP 2 - CHECK EACH fragment against ALL rules:\n"
        "a. Spelling: every single word must be a correctly-spelled English word "
        "(or a well-known proper noun/acronym like AWS, GPU). Reject ANY invented, "
        "misspelled or semi-real word (e.g. 'CLUTEP', 'PRODUCTIVIY').\n"
        "b. Legibility: no distorted, melted, overlapping or doubled letters.\n"
        "c. Grammar: each fragment must be natural, grammatically valid English.\n"
        "d. Duplicates: the SAME label must not appear more than once in the image "
        "(repeated filler labels = FAIL).\n"
        "e. Gibberish: no random character strings or pseudo-text in any language.\n"
        "If you cannot read a fragment clearly, treat it as WRONG - do not guess "
        "or give the benefit of the doubt.\n\n"
        "Answer in EXACTLY this format, nothing else:\n"
        "TEXTS: <all visible fragments, comma-separated, or 'none' if the image has no text>\n"
        "VERDICT: OK   (only if EVERY fragment passes ALL checks a-e)\n"
